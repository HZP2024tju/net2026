...
*/
void onTCPPocket(char* pkt,uint32_t dst_IP){
    ...
    if(strcmp(hostname,"server")==0){ // 自己是服务端 远端就是客户端
        local_ip = (uint32_t)inet_network(SERVER_IP);
        remote_ip = inet_network(CLIENT_IP);
    }else if(strcmp(hostname,"client")==0){ // 自己是客户端 远端就是服务端 
        local_ip = (uint32_t)inet_network(CLIENT_IP);
    ...
    if (established_socks[hashval]!=NULL){
        tju_handle_packet(established_socks[hashval], pkt,dst_IP); //这里寻址是不严谨的
        return;
    }
    ...
    if (listen_socks[hashval]!=NULL){
        tju_handle_packet(listen_socks[hashval], pkt,dst_IP);
        return;
    }
...
void* receive_thread(void* arg){
    ...
    while(1) {
        ...
        if(len >= DEFAULT_HEADER_LEN){
            ...
            // 通知内核收到一个完整的TCP报文
            onTCPPocket(pkt,from_addr.sin_addr.s_addr);
            free(pkt);
        }
    }
}
...
int cal_hash(uint32_t local_ip, uint16_t local_port, uint32_t remote_ip, uint16_t remote_port){
    // 实际上肯定不是这么算的
    return fabs(((int)local_ip+(int)local_port+(int)remote_ip+(int)remote_port)%MAX_SOCK);
}